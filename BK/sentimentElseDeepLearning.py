...
predict feature derived from and the day ahead is to be predicted) has been transferred and preprocessed further and
...
class sentimentElseDeepLearning:
    ...
    def linearModel_train(self, learning_rate=0.001, num_epochs=100):
        ...
        X_test_norm = (self.X_test - np.mean(self.X_train)) / np.std(self.X_train)
        ...
        X_predict = (self.X_predict - np.mean(self.X_train)) / np.std(self.X_train)
    ...
    def deeperLinearModel_train(self, learning_rate=0.001, num_epochs=100):
        """
    ...
    def CNN_model_train(self, learning_rate=0.001, num_epochs=50):
        ...
        model.add_module('pool1', nn.MaxPool2d(kernel_size=1))
        model.add_module(
            'conv2',
            nn.Conv2d(
        ...
        model.add_module('pool2', nn.MaxPool2d(kernel_size=1))
        model.add_module('flatten', nn.Flatten())
        model.add_module('fc1', nn.Linear(256, 32))
    ...
    def RNN_model_train(self, learning_rate=0.001, num_epochs=50):
        ...
        X_train_norm = self.X_train * 1000
        y_train = self.y_train
        X_test_norm = self.X_test * 1000
        y_test = self.y_test
        ...
        train_ds = CustomDataset(X_train_norm, y_train)
        test_ds = CustomDataset(X_test_norm, y_test)
        batch_size = 2
        ...
        torch.cuda.synchronize()
        model_cpu = model.cpu()
        X_predict = np.array([self.X_predict*1000] * 4)
