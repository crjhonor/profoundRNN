...
"""
This code will be emphasizing the use of Recurrent Neural Network.
And, before the deep learning process, dataset is generated for deep learning algorithm. Not only trading data is used ,
but also, the sentiment analysis data derived from weibo and email in this code.
Then, the prepared data, X_train, X_test, y_train, y_test and X_predict (which is used to predict one day ahead) is
transfer to the rnnDeepLearning.py and for further process and deeplearning.
"""

# # Loading Dataset into a DataLoader ----------------------------------------------------------------------------------
import pandas as pd
from sklearn.impute import KNNImputer
import rnnPreprocessing as rnnpp
...
TD_Currency_indexes = pd.read_csv('/home/crjLambda/PRO80/DailyTDs/ref_Curency.csv')
...
hot. 
"""
dataClose_logr = rnnpp.generate_logr(datasetClose)
dataClose_onehot = rnnpp.generate_onehot(datasetClose)
dataClose_multiclasses = rnnpp.generate_MultiClassesLabel(datasetClose, classes=4)
...
"""
indexList = ['CU0', "CF0", "V0", "L0"]
for x in ['PK0', 'LH0', 'PG0', 'SA0', 'EB0', 'EG0', 'SCM']:
    try:
        indexList.remove(x)
    except ValueError:
        continue
outputs_df = pd.DataFrame({'DATE': [X_predict_DATE, X_predict_DATE, X_predict_DATE],
                           'MODULE': ['RNN_1_500', 'RNN_2', 'RNN_3']})
print(X_predict_DATE)
import rnnDeepLearning
for i in tqdm(range(len(indexList)), ncols=100, desc="Recurrent Neural Network", colour="blue"):
    ind = [indexList[i]]
    label_onehot_oneDay, feature_onehot_oneDay = rnnpp.oneDaylagging(dataClose_onehot, ind, suffix='Close')
    X, y = feature_onehot_oneDay, label_onehot_oneDay
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, stratify=y, random_state=0)
    X_predict = rnnpp.oneDay_pred_feature(dataClose_onehot, ind, suffix='Close')
    sthedl = rnnDeepLearning.somethingElseDeepLearning(X_train, X_test, y_train, y_test, X_predict,
                                                       ind[0], X_predict_DATE)
    output = []
    output.append(sthedl.RNN_model_train_BCEloss(learning_rate=0.001, num_epochs=250))
    output.append(sthedl.RNN_model_train_BCEWithLogitsLoss(learning_rate=0.001, num_epochs=250))
    output.append(sthedl.RNN_model_train_BCEWithLogitsLoss(learning_rate=0.001, num_epochs=10))
...
outputs_filename = Path(Path(os.getcwd()).parents[1], dataDirName, "rnnDicinger_prob_tbl.csv")
