...
class simpleCNN(paddle.nn.Layer):
    def __init__(self):
        ...
        self.fc = Linear(in_features=60, out_features=1)
...
class simpleConvolutionnetwork:
    ...
    def scnn_train(self, datas): # features and labels
        ...
        opt = paddle.optimizer.SGD(learning_rate=0.01, parameters=scnn_model.parameters())
        EPOCH_NUM = 50
        features = datas[0]
        labels = datas[1]
        for epoch in range(EPOCH_NUM):
            for i in range(labels.shape[0]):
                feature = features[i].reshape((1, 1, 2, 4)).astype('float32')
    ...
    def scnn_predict(self, datas):
        # for day t+1
        feature_t_1 = datas[0]
        feature_t_1_tensor = paddle.to_tensor(feature_t_1).reshape((1, 1, 2, 4)).astype('float32')
        ...
        # for day t+2
        feature_t_2 = datas[1]
        feature_t_2_tensor = paddle.to_tensor(feature_t_2).reshape((1, 1, 2, 4)).astype('float32')
