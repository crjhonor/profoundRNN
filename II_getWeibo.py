...
UID: 5044281310 paper.cn
UID: 1663937380 CAIXIN
...
URL: https://weibo.com/ajax/statuses/mymblog?uid=1875034341&page=1&feature=0
"""
...
class mariaDBstorage:
    ...
    def getTodate(self):
        ...
        try:
            ...
            toDate = pd.to_datetime('2023-01-15')
...
class CrawlWeibo:
        ...
        while True:
            ...
            try:
                for i in ob_json['data']['cards']:
                    #print(pd.to_datetime(i['mblog']['created_at']).date())
    ...
    def storeTodatabase(self):
        ...
        fromDate = pd.to_datetime(self.fromDate)
        toDate = self.toDate
        mode = self.mode
        if mode == 'startup':
            for page in results:
...
uid = '5044281310'
cl_three = CrawlWeibo(uid, fromDate, toDate, mode='update')
uid = '1663937380'
cl_four = CrawlWeibo(uid, fromDate, toDate, mode='update')
